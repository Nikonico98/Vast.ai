...
export async function processFullPhotoEvent(journeyId, photoFile, onProgress, world) {
  ...
  try {
    ...
    // Update Step 1 with analysis results (typewriter will handle display)
    ...
    // Deliberate pause — let user read analysis results (typewriter ~2s)
    await new Promise(r => setTimeout(r, 2500));

    // Step 2: Fictional Event Generation (Story + Item/Character)
    ...
    // Event was already generated by backend, update with results
    ...
    if (CONFIG.SKIP_3D_GENERATION) {
      ...
      // Update Step 4A: Photo 3D
      onProgress?.("3d-photo", "Skipped (test mode)", {
        status: "completed",
        progress: 100,
      });

      // Update Step 4B: Fictional 3D
    ...
    // Step 3: Generate fictional image (already done in processPhotoEvent)
    onProgress?.("fictional-image", getStepMsg(world, "imageComplete"), {
      status: "completed",
      progress: 100,
      fictionalImageUrl: event.fictionalImageUrl,
